...
class MPCCNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('R_vel', 0.1)
        self.declare_parameter('R_delta', 1.0)
        self.declare_parameter('Q_prog', 1.0)
        self.declare_parameter('Q_norm_K', 0.1)
        ...
        try:
            ...
            self.get_logger().info(f"centerline loaded....") 
            self.build_casadi_interpolants()
            self.get_logger().info("CasADi interpolants ready....")
        except Exception as e:
            self.get_logger().error(f"Failed to load/build centerline: {e}")
    ...
    def read_centerline_csv(self):

        df = pd.read_csv(self.centerline_path, 
                        delimiter=',', 
    ...
    def timer_callback(self):
        ...
        if s_est is None:
            self.get_logger().warn("Could not find nearest s!")
